...
- 由 showUndo 觸發顯示
- 觸發來源:
    - 交易編輯器的新增、編輯、刪除，含一般與定期，定期含僅此一筆與此筆及未來
    - 轉帳編輯器的新增、編輯、刪除，含一般與定期
    - 帳戶編輯器的新增、編輯、刪除
    - 類別編輯器的新增、編輯、刪除
    - 合併編輯器完成合併
- 上述操作皆發生於編輯器畫面，編輯器關閉後 Undo Bar 覆蓋於返回的目的畫面，即首頁或對應列表
...
- 復原段：倒數計時與固定復原標籤並列，整段為單一觸發區
- 復原標籤顯示在地化的復原字樣，不隨操作類型變動
- 取消段：取消動作
...
- **要求:**
    - 編輯器 screen 在寫入操作成功處標註 `呼叫 showUndo`
    - 不重述 Undo Bar 結構與互動
