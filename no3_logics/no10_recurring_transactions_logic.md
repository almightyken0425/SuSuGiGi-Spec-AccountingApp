...
## convertToSchedule 單筆轉排程

- 把既有的一般交易或轉帳轉為週期排程
- 順序固定為先建排程、後刪原紀錄，杜絕資料遺失視窗
- **輸入:**
  - 紀錄類型，交易或轉帳
  - 原紀錄識別碼
  - 排程資料
...
- **執行:**
  ...
  - **IF** 建立失敗:
    ...
    - 原紀錄保持完好
  - **IF** 建立成功:
    - 依紀錄類型呼叫 deleteTransaction 或 deleteTransfer
    - **IF** 刪除失敗:
      - 容忍失敗，不回滾已建立的排程
      - 原紀錄殘留優於排程資料遺失
- **回傳:**
...
## restoreScheduleInstances 復原排程實例
...
## revertScheduleCreation 撤銷排程建立

- 供復原動作反向撤掉一次排程建立，含排程本體與其名下實例
- **輸入:**
  - 排程識別碼
- **執行:**
  - 查詢該排程名下所有未軟刪除的實例
  - 軟刪除排程本體與查得的實例，單次原子提交

---

## revertConvertToSchedule 撤銷單筆轉排程

- 供復原動作反向撤掉一次 convertToSchedule
- 全部變更單次原子提交
- 原子性對應正向的資料保全政策：不可出現原紀錄與排程雙亡的中間態
- **輸入:**
  - 新排程識別碼
  - 原紀錄
- **執行:**
  - 清除原紀錄的軟刪除標記
  - 軟刪除新排程本體與其名下所有未軟刪除的實例
  - **IF** 正向轉換時原紀錄刪除曾被容忍失敗:
    - 清除標記落在未刪紀錄上，無害

---

## revertScheduleUpdate 撤銷排程更新

- 供復原動作反向撤掉一次此筆及未來的排程更新
- 全部變更單次原子提交，不留半套復原
- **輸入:**
  - 新排程識別碼，未建新排程時為空
  - 原排程
